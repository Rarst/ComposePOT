...
/**
 * Generates POT file from the sources.
 */
class MakePOT {
	...
	function xgettext( $project, $dir, $output_file, $placeholders = array(), $excludes = array(), $includes = array() ) {
		...
		if ( ! empty( $meta['package-name'] ) ) {
			$project_id_version .= $meta['package-name'];
		}

		if ( ! empty( $meta['package-version'] ) ) {
			$project_id_version .= ' ' . $meta['package-version'];
		}

		$pot->set_header( 'Project-Id-Version', $project_id_version );
